...
export const updateTheProduct = (
    id, name, productType, sizes, oldCover, coverImage, oldExtra, extraImages, extraList, 
    productPrice, shippingPrice, quantity, description, discountPercentage, discountCode) => async dispatch => {
    try {
        const token = ls.get("authToken")
        ...
        for(var i = 0; i < oldExtra.length; i++){
            const deleteExtra = await deleteImages(oldExtra[i]);
            if(!deleteExtra){
...
export const setNotShipping = (id) => async dispatch => {
    const token = ls.get("authToken")

    try {
        const res = await axios.post("/api/setNotShipped",
        {id:id},
        {
           headers:{
           authorization: 'Bearer ' + token
    ...
    for(var i = 0; i < extraImages.length; i++){
        ...
        if(result1.message === "successfully uploaded"){
            ...
            for(var i = 0; i < extraImagesList.length; i++){
                const re = await deleteImages(extraImagesList[i]);
                if(!re){
            ...
            if(remaining.length != 0){
                for(var i = 0; i < remaining.length; i++){
                    await deleteImages(extraImagesList[remaining[i]]);
                }
            }
